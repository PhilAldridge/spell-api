...
func (_q *GroupQuery) QuerySchool() *SchoolQuery {
...
func (_q *GroupQuery) QueryWordLists() *WordListQuery {
...
func (_q *GroupQuery) QueryCompetitions() *CompetitionQuery {
...
func (_q *GroupQuery) WithSchool(opts ...func(*SchoolQuery)) *GroupQuery {
	query := (&SchoolClient{config: _q.config}).Query()
...
func (_q *GroupQuery) WithWordLists(opts ...func(*WordListQuery)) *GroupQuery {
	query := (&WordListClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) WithCompetitions(opts ...func(*CompetitionQuery)) *GroupQuery {
	query := (&CompetitionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withSchool; query != nil {
		if err := _q.loadSchool(ctx, query, nodes, nil,
			func(n *Group, e *School) { n.Edges.School = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withWordLists; query != nil {
		if err := _q.loadWordLists(ctx, query, nodes,
			func(n *Group) { n.Edges.WordLists = []*WordList{} },
			func(n *Group, e *WordList) { n.Edges.WordLists = append(n.Edges.WordLists, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCompetitions; query != nil {
		if err := _q.loadCompetitions(ctx, query, nodes,
			func(n *Group) { n.Edges.Competitions = []*Competition{} },
			func(n *Group, e *Competition) { n.Edges.Competitions = append(n.Edges.Competitions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GroupQuery) loadSchool(ctx context.Context, query *SchoolQuery, nodes []*Group, init func(*Group), assign func(*Group, *School)) error {
...
func (_q *GroupQuery) loadWordLists(ctx context.Context, query *WordListQuery, nodes []*Group, init func(*Group), assign func(*Group, *WordList)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Group)
	nids := make(map[int]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.WordListsTable)
		s.Join(joinT).On(s.C(wordlist.FieldID), joinT.C(group.WordListsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.WordListsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.WordListsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *GroupQuery) loadCompetitions(ctx context.Context, query *CompetitionQuery, nodes []*Group, init func(*Group), assign func(*Group, *Competition)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Group)
	nids := make(map[int]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.CompetitionsTable)
		s.Join(joinT).On(s.C(competition.FieldID), joinT.C(group.CompetitionsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.CompetitionsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.CompetitionsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
