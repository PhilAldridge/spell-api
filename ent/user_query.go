...
func (_q *UserQuery) QuerySchools() *SchoolQuery {
...
// QueryOwnedSchool chains the current query on the "owned_school" edge.
func (_q *UserQuery) QueryOwnedSchool() *SchoolQuery {
	query := (&SchoolClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withSchools:       _q.withSchools.Clone(),
		withOwnedSchool:   _q.withOwnedSchool.Clone(),
...
func (_q *UserQuery) WithSchools(opts ...func(*SchoolQuery)) *UserQuery {
	query := (&SchoolClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSchools = query
	return _q
}

// WithOwnedSchool tells the query-builder to eager-load the nodes that are connected to
// the "owned_school" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithOwnedSchool(opts ...func(*SchoolQuery)) *UserQuery {
	query := (&SchoolClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOwnedSchool = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withSchools; query != nil {
		if err := _q.loadSchools(ctx, query, nodes,
			func(n *User) { n.Edges.Schools = []*School{} },
			func(n *User, e *School) { n.Edges.Schools = append(n.Edges.Schools, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withOwnedSchool; query != nil {
		if err := _q.loadOwnedSchool(ctx, query, nodes, nil,
			func(n *User, e *School) { n.Edges.OwnedSchool = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadSchools(ctx context.Context, query *SchoolQuery, nodes []*User, init func(*User), assign func(*User, *School)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.SchoolsTable)
		s.Join(joinT).On(s.C(school.FieldID), joinT.C(user.SchoolsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.SchoolsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.SchoolsPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*School](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "schools" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *UserQuery) loadOwnedSchool(ctx context.Context, query *SchoolQuery, nodes []*User, init func(*User), assign func(*User, *School)) error {
	...
	query.Where(predicate.School(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.OwnedSchoolColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_owned_school
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_owned_school" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_owned_school" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
