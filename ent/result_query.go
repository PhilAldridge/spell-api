...
func (_q *ResultQuery) QueryUser() *UserQuery {
...
func (_q *ResultQuery) QueryWord() *WordQuery {
...
func (_q *ResultQuery) WithUser(opts ...func(*UserQuery)) *ResultQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *ResultQuery) WithWord(opts ...func(*WordQuery)) *ResultQuery {
	query := (&WordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ResultQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Result, error) {
	...
	if _q.withUser != nil || _q.withWord != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *Result, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withWord; query != nil {
		if err := _q.loadWord(ctx, query, nodes, nil,
			func(n *Result, e *Word) { n.Edges.Word = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ResultQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Result, init func(*Result), assign func(*Result, *User)) error {
...
func (_q *ResultQuery) loadWord(ctx context.Context, query *WordQuery, nodes []*Result, init func(*Result), assign func(*Result, *Word)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Result)
	for i := range nodes {
		if nodes[i].result_word == nil {
			continue
		}
		fk := *nodes[i].result_word
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "result_word" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
